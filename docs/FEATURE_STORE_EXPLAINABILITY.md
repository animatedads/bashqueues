# Feature Store explainability notes

The `feature_store` provider family is intended to explain provider capability and governance facts without executing service operations. Provider helpers must never return shell commands or command fragments. queuebash and reviewers consume the JSON evidence as advisory context only.
