# Edge CDN explainability

The `edge_cdn` provider helper produces small normalized JSON documents that explain detected capability, dependency, policy and safety posture. The documents are fixtures by default so tests can reason about service estates without credentials or live calls.
