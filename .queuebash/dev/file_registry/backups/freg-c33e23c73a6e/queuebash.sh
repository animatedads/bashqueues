...
_queue_ai_ask_provider_known() {
    case "${1:-}" in
        contract|fixture|ollama|gemini) return 0 ;;
    esac
    local source_dir
    source_dir="$(_queue_ai_source_dir 2>/dev/null || pwd)"
...
_queue_ai_provider_requires_network() {
    case "${1:-}" in
        gemini|openai|anthropic|watsonx|bedrock|azure_ai|vertex_ai) echo true ;;
        *) echo false ;;
    esac
}
...
_queue_ai_provider_supports_json() {
    case "${1:-}" in
        contract|fixture|ollama|gemini) echo true ;;
        *) echo false ;;
    esac
}
...
_queue_ai_provider_list() {
    local source_dir f base seen=" contract fixture gemini ollama "
    printf '%s\n' contract fixture gemini ollama
    source_dir="$(_queue_ai_source_dir 2>/dev/null || pwd)"
...
Usage:
  ...
  queue dev files begin|finish|add|remove|list|changed|path
...
KINDS = {"contract", "design_goal", "architecture", "task", "attempt", "evidence", "failure", "success", "decision", "toolchain", "known_landmine", "blocker", "challenge", "done_note", "imported_fact"}
...
def snapshot(file,functions=None):
    ...
    fmap=function_map(p); wanted=functions or sorted(fmap); fl=[fmap[x] for x in wanted if x in fmap]
...
def update_change(entry, functions=None):
    cur=snapshot(entry['path'], functions); base=entry.get('baseline',{})
    ...
    is_changed=base.get('md5') != cur.get('md5')
...
def cmd_add():
    ...
    data=load(); snap=snapshot(ns.file,ns.function); entry=find_entry(data,snap['path']) or {'schema':ENTRY_SCHEMA,'id':'freg-'+uuid.uuid4().hex[:12],'created_at':now(),'baseline':snap}
    if entry not in data['entries']: data['entries'].append(entry)
    entry.update({'updated_at':now(),'status':'changed','path':snap['path'],'relpath':snap['relpath'],'location':ns.location or snap['relpath'],'purpose':ns.purpose,'current':snap,'changed':True}); update_change(entry,ns.function or None); event(data,'add',entry,purpose=ns.purpose); save(data); emit({'schema':ENTRY_SCHEMA,'status':'added','entry_id':entry['id'],'relpath':snap['relpath'],'md5':snap['md5'],'size':snap['size']},ns.json)
def cmd_remove():
...
try:
    if sub in {'help','--help','-h',''}: print('Usage: queue dev files begin|finish|add|remove|list|changed|path', file=sys.stderr); sys.exit(0)
    {'path':cmd_path,'begin':cmd_begin,'finish':cmd_finish,'add':cmd_add,'remove':cmd_remove,'list':lambda:cmd_list(False),'changed':lambda:cmd_list(True)}[sub]()
except KeyError:
...
_queue_dev_patchset_command() {
    local sub="${1:-}" registry="" output="" json=0
    shift || true
    [[ "$sub" == "create" ]] || { echo "Usage: queue dev patchset create --output ZIP [--registry FILE] [--json]" >&2; return 2; }
    while [[ "$#" -gt 0 ]]; do
        case "${1:-}" in
    ...
    python3 - "$registry" "$output" "$json" <<'PYDEV_PATCHSET'
...
data=json.loads(registry.read_text()); entries=[e for e in data.get('entries',[]) if e.get('changed') and e.get('status')=='changed']
...
def safe_rel(rel):
    ...
    return str(p)
manifest={'schema':'queuebash.dev_patchset.v1','created_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'source_root':str(root),'registry':str(registry),'entries':[]}
...
try:
    ...
    for e in entries:
        ...
        backup=pathlib.Path(e.get('backup','')); diff_rel=f'diffs/{rel}.diff'; diff_path=work/diff_rel; diff_path.parent.mkdir(parents=True,exist_ok=True)
        if backup.exists():
        ...
        else: diff_path.write_text(f'# baseline backup missing for {rel}\n')
        base=e.get('baseline',{}); cur=e.get('current',{})
        manifest['entries'].append({'entry_id':e.get('id'),'relpath':rel,'purpose':e.get('purpose'),'file_old_md5':base.get('md5'),'file_new_md5':md5_file(src),'file_old_size':base.get('size'),'file_new_size':src.stat().st_size,'changed_functions':e.get('changed_functions',[]),'diff':diff_rel,'file':f'files/{rel}','baseline_present':backup.exists()})
    ...
    (work/'apply_patchset.sh').write_text('#!/usr/bin/env bash\nset -euo pipefail\nhere="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"\ntarget="${1:-.}"\npython3 "$here/scripts/check_preconditions.py" "$here/manifest.json" "$target"\npython3 "$here/scripts/apply_files.py" "$here/manifest.json" "$target" "$here/files"\n')
    (work/'scripts'/'check_preconditions.py').write_text(r'''#!/usr/bin/env python3
import hashlib,json,pathlib,re,sys
manifest=pathlib.Path(sys.argv[1]); target=pathlib.Path(sys.argv[2]); data=json.loads(manifest.read_text())
def md5_file(path):
 h=hashlib.md5()
 with open(path,'rb') as f:
  for c in iter(lambda:f.read(1024*1024), b''): h.update(c)
...
failed=False
for e in data.get('entries',[]):
 path=target/e['relpath']
 if not path.exists(): print(f"MISSING {e['relpath']}"); failed=True; continue
 old=e.get('file_old_md5')
 if old and md5_file(path)!=old:
  f=funcs(path); bad=[]
  ...
  if bad: print(f"PRECONDITION FAILED {e['relpath']}: file md5 differs and function baseline mismatch: {'; '.join(bad)}"); failed=True
  else: print(f"WARN {e['relpath']}: file md5 differs, changed-function old md5 preconditions match")
 else: print(f"OK {e['relpath']}: file baseline md5 matched")
sys.exit(1 if failed else 0)
''')
    (work/'scripts'/'apply_files.py').write_text("#!/usr/bin/env python3\nimport json,pathlib,shutil,sys\ndata=json.loads(pathlib.Path(sys.argv[1]).read_text()); target=pathlib.Path(sys.argv[2]); files=pathlib.Path(sys.argv[3])\nfor e in data.get('entries',[]):\n rel=e['relpath']; dst=target/rel; dst.parent.mkdir(parents=True,exist_ok=True); shutil.copy2(files/rel,dst); print(f'applied {rel}')\n")
...
if json_mode: print(json.dumps({'schema':'queuebash.dev_patchset.v1','status':'ok','output':str(outzip),'entries':len(entries)},sort_keys=True,separators=(',',':')))
