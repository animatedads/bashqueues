...
_queue_dev_patchset_command() {
    local sub="${1:-}" registry="" output="" patchset="" target="" json=0
    shift || true
    case "$sub" in
        create)
            while [[ "$#" -gt 0 ]]; do
...
manifest={'schema':'queuebash.dev_patchset.v1','created_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'source_root':str(root),'registry':str(registry),'summary':{'total_entries':0,'modified_files':0,'new_or_unbaselined_files':0,'missing_baseline_backups':0,'function_preconditions':0},'entries':[]}
...
try:
    ...
    for e in entries:
        ...
        change_type='new_or_unbaselined_file' if old_md5 is None else 'modified_file'
        ...
        manifest['entries'].append({'entry_id':e.get('id'),'relpath':rel,'purpose':e.get('purpose'),'change_type':change_type,'file_old_md5':old_md5,'file_new_md5':new_md5,'file_old_size':base.get('size'),'file_new_size':src.stat().st_size,'changed_functions':e.get('changed_functions',[]),'diff':diff_rel,'file':f'files/{rel}','baseline_present':baseline_present,'precondition':{'file_old_md5_required':old_md5 is not None,'allow_absent_target_for_new_file':old_md5 is None,'allow_matching_existing_new_file':old_md5 is None}})
...
summary={'total':len(results),'ready':0,'already_applied':0,'conflict':0,'missing':0,'requires_full_file_reconciliation':0}
for r in results:
 st=r['status']
        ...
        inspect)
            while [[ "$#" -gt 0 ]]; do
                case "${1:-}" in
                    ...
                    --json|-j) json=1; shift ;;
                    *) echo "queue dev patchset inspect: unexpected argument: $1" >&2; return 2 ;;
                esac
            done
            [[ -n "$patchset" ]] || { echo "Usage: queue dev patchset inspect --patchset ZIP [--target DIR] [--json]" >&2; return 2; }
            python3 - "$patchset" "${target:-}" "$json" <<'PYDEV_PATCHSET_INSPECT'
import json, pathlib, subprocess, sys, tempfile, zipfile, shutil
patchset=pathlib.Path(sys.argv[1]); target=sys.argv[2]; json_mode=sys.argv[3]=='1'
if not patchset.exists(): print(f'queue dev patchset inspect: patchset not found: {patchset}', file=sys.stderr); sys.exit(1)
work=pathlib.Path(tempfile.mkdtemp(prefix='queue-dev-patchset-inspect.'))
out={}
try:
    with zipfile.ZipFile(patchset) as z: z.extractall(work)
    manifest=json.loads((work/'manifest.json').read_text())
    out={'schema':'queuebash.dev_patchset.inspect.v1','status':'ok','patchset':str(patchset),'summary':manifest.get('summary',{}),'entries':manifest.get('entries',[])}
    if target:
        proc=subprocess.run([sys.executable,str(work/'scripts'/'check_preconditions.py'),str(work/'manifest.json'),target,'--json'],text=True,capture_output=True,timeout=15)
        try: out['preconditions']=json.loads(proc.stdout or '{}')
        except Exception: out['preconditions']={'status':'failed','stdout':proc.stdout,'stderr':proc.stderr,'returncode':proc.returncode}
        if proc.returncode != 0: out['status']='precondition_failed'
    if json_mode: print(json.dumps(out,sort_keys=True,separators=(',',':')))
    else:
        s=out.get('summary',{})
        print('Patchset summary: '+', '.join(f'{k}={v}' for k,v in sorted(s.items())))
        for e in out.get('entries',[]): print(f"{e.get('change_type','changed')}\t{e.get('relpath')}\told={e.get('file_old_md5')}\tnew={e.get('file_new_md5')}")
        if 'preconditions' in out: print('Preconditions: '+out['preconditions'].get('status','unknown'))
finally:
    shutil.rmtree(work,ignore_errors=True)
sys.exit(0 if out.get('status')!='precondition_failed' else 1)
PYDEV_PATCHSET_INSPECT
            ;;
        help|--help|-h|"") echo "Usage: queue dev patchset create --output ZIP [--registry FILE] [--json]" >&2; echo "       queue dev patchset inspect --patchset ZIP [--target DIR] [--json]" >&2; return 0 ;;
