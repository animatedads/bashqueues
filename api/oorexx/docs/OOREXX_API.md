...
| `BQSurface` | Lazily builds an introspected command map from `queue dev functions --json` when `q~surface`, `q~discover`, or `BASHQUEUES_OOREXX_DISCOVER=1` is used. |
...
```rexx
functions = q~dev~functions~json      /* can be slow on large trees */
