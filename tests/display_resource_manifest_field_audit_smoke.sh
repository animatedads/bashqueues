...
OUT="$(mktemp)"
trap 'rm -f "$OUT"' EXIT
python3 "$ROOT/bin/queue-display-resource-manifest-field-audit.py" --root "$ROOT" --json > "$OUT"
python3 - "$OUT" <<'PY'
import json, sys
...
assert data['summary']['audited_entries'] >= 1
...
assert data['json_contract_source'] is False
PY
echo "display_resource_manifest_field_audit_smoke: ok"
