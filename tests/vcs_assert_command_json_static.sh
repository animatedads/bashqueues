...
bash -n classes/VCS_CHANGESET_AUDIT.env || fail 'VCS_CHANGESET_AUDIT syntax failed'
...
grep -q '"helpers":\["queue-vcs-detect","queue-vcs-probe","queue-vcs-assert","queue-vcs-baseline"\]' queuebash.sh || fail 'queue vcs types JSON missing helper metadata'
...
grep -q 'queue vcs assert \[PATH\]' resources.d/display/lang_eng/vcs-help.txt || fail 'display help missing assert usage'
