...
grep -q 'QueueManager commands' <<< "$queue_help" || fail "queue mgr help wrong"
grep -q 'QueueManager commands' <<< "$bare_help" || fail "bare queuemgr help wrong"
grep -q 'QueueManager commands' <<< "$manager_help" || fail "queue manager help wrong"

type queuemgr | grep -q 'queue mgr "$@"' || fail "bare queuemgr is not wrapper"

type _queue_legacy_queuemgr >/dev/null 2>&1 || fail "legacy manager function missing"

pass "bare queuemgr routes to new QueueManager"
pass "queue manager routes to new QueueManager"
pass "legacy manager is isolated"
