...
probe="$(bash bin/queue-vcs-probe --json --timeout 1 "$repo")"
...
queue_asset_check_vcs_identity _tok "$repo" type=cvs require_identity=REL_1_0 timeout=1 | grep -q 'asset_check_ok' || fail "vcs identity check failed for CVS sticky tag"
queue_asset_check_vcs_revision _tok "$repo" type=cvs require_revision=/cvs/root timeout=1 | grep -q 'asset_check_ok' || fail "vcs revision check failed for CVS root metadata"
