...
set -euo pipefail
cd "$(dirname "$0")/.."
helper="providers.d/eu_sovereign/eu_sovereign_provider.sh"
fixture="tests/fixtures/eu_sovereign"
tmpdir="${TMPDIR:-/tmp}/eu-sovereign-provider-smoke.$$"
mkdir -p "$tmpdir"
trap 'rm -rf "$tmpdir"' EXIT
run_json(){ QUEUEBASH_EU_SOVEREIGN_FIXTURE_DIR="$fixture" "$helper" "$@"; }
...
for p in providers:
  for c in checks:
    ...
    assert data['provider_family']=='eu_sovereign'
    assert data['provider']==p
    assert data['schema']==f'queuebash.eu_sovereign.{p}.{c}.v1'
    if 'decision' in data:
      assert data['decision'] in ('allow','available','deny','unknown')
    text=json.dumps(data)
    forbidden=['api_secret_value','PRIVATE KEY','signedUrl','signed_url_value','token_value','console_session']
  ...
  assert json.loads((base/f'{p}-identity.json').read_text())['api_secret_stored'] is False
  assert json.loads((base/f'{p}-legal.json').read_text())['status']=='mapped_pending_validation'
print('PASS eu sovereign fixture json')
PY
echo 'PASS eu_sovereign_provider_fixture_smoke'
