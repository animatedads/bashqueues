...
doc = (root / "docs" / "OCI_GOVERNANCE_PROVIDER.md").read_text(encoding="utf-8")
blocks = re.findall(r"```json\n(.*?)\n```", doc, flags=re.S)
if len(blocks) < 7:
    raise SystemExit("[FAIL] expected at least seven JSON contract examples")
...
for block in blocks:
    ...
    if obj.get("provider") != "oci":
        raise SystemExit("[FAIL] JSON example missing provider oci")
    ...
    if obj.get("auth_mode") == "instance_principal":
        seen_auth = True
    ...
    if "posture" in obj:
        seen_posture = True
    if "evidence_bundle_ref" in obj:
        seen_audit = True

if not seen_auth:
    raise SystemExit("[FAIL] no auth context example found")
...
if not seen_deny:
    raise SystemExit("[FAIL] no deny example found")
if not seen_key:
    raise SystemExit("[FAIL] no key resolver example found")
if not seen_posture:
    raise SystemExit("[FAIL] no posture example found")
if not seen_audit:
    raise SystemExit("[FAIL] no audit evidence example found")

print("[PASS] OCI governance provider JSON examples parse and validate")
