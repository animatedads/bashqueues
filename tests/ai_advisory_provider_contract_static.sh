...
grep -q 'QUEUEBASH_VERSION="0.18.0"' queuebash.sh || fail 'version not bumped to 0.18.0'
grep -q '0.18.0 - AI advisory provider contract' CHANGELOG.md || fail 'changelog entry missing'
...
! grep -R '/etc/bashqueues' docs/AI_ADVISORY_PROVIDER.md docs/AI_AUDIT_LOGGING.md examples/providers/ai/watson.env.example || fail 'legacy /etc/bashqueues namespace drift'
